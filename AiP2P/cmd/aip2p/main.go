...
func runIdentityInit(args []string) error {
	...
	out := fs.String("out", "", "identity file output path; defaults to ~/.aip2p-sharing/identities/<sanitized-agent-id>.json")
...
func runServe(args []string) error {
	...
	versionLabel := fmt.Sprintf("host version label; defaults to %s", version.Current)
	version := fs.String("version", "", versionLabel)
...
func defaultIdentityOutputPath(agentID, explicitOut string) (string, error) {
	...
	return filepath.Join(home, ".aip2p-sharing", "identities", sanitizeAgentIDForFilename(agentID)+".json"), nil
}
