...
func PluginFiles(name string) ([]File, error) {
	...
	return []File{
		...
		{Path: "README.md", Content: fmt.Sprintf("# %s\n\nThis is an AiP2P plugin scaffold.\n\nThe generated manifest is immediately runnable as a third-party directory plugin by delegating to the built-in `news-demo-content` runtime through `base_plugin`.\n\nTry it with:\n\n`aip2p serve --plugin-dir . --theme news-demo`\n", strings.TrimSpace(name))},
...
func AppFiles(name string) ([]File, error) {
	id := Slug(name)
	basePluginID := "news-demo-content"
	pluginID := id + "-plugin"
	themeID := id + "-theme"
	projectID := id + ".sample"
	channel := id + "/general"
