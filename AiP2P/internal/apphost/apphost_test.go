...
func TestRegistryKeepsSharedContentPathsForBasePluginWrappers(t *testing.T) {
	registry := NewRegistry()
	registry.MustRegisterTheme(themeWithManifest{
		manifest: ThemeManifest{
			ID:               "test-theme",
			Name:             "Test Theme",
			SupportedPlugins: []string{"sample-content", "news-demo-content"},
		},
	})
	var wrappedCfg Config
	registry.MustRegisterPlugin(pluginWithManifest{
		...
		build: func(_ context.Context, cfg Config, _ WebTheme) (*Site, error) {
			wrappedCfg = cfg
			return &Site{Handler: http.HandlerFunc(func(http.ResponseWriter, *http.Request) {})}, nil
		},
	})
	...
	if err != nil {
		t.Fatalf("build wrapper site: %v", err)
	}

	if wrappedCfg.RuntimeRoot != "/tmp/runtime" {
		t.Fatalf("runtime root = %q", wrappedCfg.RuntimeRoot)
	}
	if wrappedCfg.StoreRoot != "/tmp/store" {
		t.Fatalf("store root = %q", wrappedCfg.StoreRoot)
	}
	if wrappedCfg.ArchiveRoot != "/tmp/archive" {
		t.Fatalf("archive root = %q", wrappedCfg.ArchiveRoot)
	}
	if wrappedCfg.RulesPath != "/tmp/config/subscriptions.json" {
		t.Fatalf("rules path = %q", wrappedCfg.RulesPath)
	}
}
