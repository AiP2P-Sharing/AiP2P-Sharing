...
func BuildArchiveDayStats(entries []ArchiveEntry) []SummaryStat {
	...
	return []SummaryStat{
		...
		{Label: "Assets", Value: strconv.Itoa(stories)},
...
func BuildArchiveEntries(index Index, day string) []ArchiveEntry {
	...
	for _, bundle := range index.Bundles {
		...
		entries = append(entries, archiveEntry(index, bundle))
	}
...
func FindArchiveEntry(index Index, infoHash string) (ArchiveEntry, bool) {
	...
	for _, bundle := range index.Bundles {
		if strings.ToLower(bundle.InfoHash) == infoHash && bundle.ArchiveMD != "" {
			return archiveEntry(index, bundle), true
		}
	}
	return ArchiveEntry{}, false
}

func archiveEntry(index Index, bundle Bundle) ArchiveEntry {
	title := strings.TrimSpace(bundle.Message.Title)
	if title == "" {
	...
	return ArchiveEntry{
		...
		Day:        day,
		ThreadURL:  bundleThreadURL(index, bundle),
...
func bundleThreadURL(index Index, bundle Bundle) string {
	switch bundle.Message.Kind {
	case "post":
		post := Post{Bundle: bundle}
		if path := CoordCanonicalPath(post); path != "" {
			return path
		}
		return "/posts/" + bundle.InfoHash
	case "reply":
		if bundle.Message.ReplyTo != nil && bundle.Message.ReplyTo.InfoHash != "" {
			if path := canonicalPathForInfoHash(index, bundle.Message.ReplyTo.InfoHash); path != "" {
				return path
			}
	...
	case "reaction":
		if infoHash := nestedString(bundle.Message.Extensions, "subject", "infohash"); infoHash != "" {
			if path := canonicalPathForInfoHash(index, infoHash); path != "" {
				return path
			}
...
func canonicalPathForInfoHash(index Index, infoHash string) string {
