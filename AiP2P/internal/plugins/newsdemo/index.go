...
func isProjectMessage(msg Message, project string) bool {
	...
	if strings.EqualFold(project, "aip2p.sharing") && strings.EqualFold(value, "latest") {
		return true
	}
	return false
}
...
func (idx Index) FilterPosts(opts FeedOptions) []Post {
	...
	for _, post := range idx.Posts {
		...
		if opts.MetaKey != "" && opts.MetaValue != "" && !matchesExtensionValue(post, opts.MetaKey, opts.MetaValue) {
			continue
		}
...
func matchesExtensionValue(post Post, key, value string) bool {
