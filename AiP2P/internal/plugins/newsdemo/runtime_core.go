...
func (a *App) pageNav(activePath string) []NavItem {
	items := make([]NavItem, 0, 11)
	items = append(items, NavItem{Name: "Home", URL: "/", Active: activePath == "/"})
	for _, spec := range CoordCollections() {
		items = append(items, NavItem{Name: spec.NavName, URL: spec.Path, Active: strings.HasPrefix(activePath, spec.Path)})
	}
	items = append(items,
...
func displayProjectName(project string) string {
	if strings.EqualFold(strings.TrimSpace(project), "aip2p.sharing") {
		return "AiP2P Sharing"
	}
