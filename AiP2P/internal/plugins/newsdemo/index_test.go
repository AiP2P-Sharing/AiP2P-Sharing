...
func TestBuildIndexPrefersOriginPublicKeyForSourceGrouping(t *testing.T) {
	...
	index := buildIndex(bundles, "aip2p.sharing")
...
func TestFilterPostsSupportsMetadataFilter(t *testing.T) {
	t.Parallel()
	...
	index := Index{
		Posts: []Post{
			...
			},
			{
				Bundle: Bundle{
					InfoHash:  "active",
					CreatedAt: time.Date(2026, 3, 17, 11, 0, 0, 0, time.UTC),
					Message: Message{
						Title: "Active task",
						Extensions: map[string]any{
	...
	if got[0].InfoHash != "active" {
		t.Fatalf("infohash = %s, want active", got[0].InfoHash)
	}
}
...
func TestLoadIndexMissingStoreReturnsEmpty(t *testing.T) {
	...
	index, err := LoadIndex(root, "aip2p.sharing")
	if err != nil {
