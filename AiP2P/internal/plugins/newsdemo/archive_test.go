...
func TestBundleThreadURLUsesCanonicalTypedPathForPost(t *testing.T) {
	t.Parallel()
	...
	if got := bundleThreadURL(index, bundle); got != "/ideas/idea-1" {
		t.Fatalf("bundleThreadURL() = %q, want /ideas/idea-1", got)
	}
}

func TestBundleThreadURLUsesCanonicalTypedPathForReplySubject(t *testing.T) {
	t.Parallel()
	...
	if got := bundleThreadURL(index, bundle); got != "/tasks/task-1" {
		t.Fatalf("bundleThreadURL() = %q, want /tasks/task-1", got)
	}
}

func TestBundleThreadURLFallsBackForUnknownReplySubject(t *testing.T) {
	t.Parallel()

	index := Index{PostByInfoHash: map[string]Post{}}
	bundle := Bundle{
		InfoHash: "reply-1",
		Message: Message{
			Kind: "reply",
	...
	if got := bundleThreadURL(index, bundle); got != "/posts/task-1" {
		t.Fatalf("bundleThreadURL() = %q, want /posts/task-1", got)
	}
}
