...
const projectSyncBinaryName = "aip2p-sharing-syncd"
...
func defaultLatestNetINF() (string, error) {
	...
	return fmt.Sprintf(`# AiP2P Sharing bootstrap configuration
# Plaintext file loaded by --net ~/.aip2p-sharing/aip2p_sharing_net.inf
#
# Supported keys:
...
#   libp2p_rendezvous=aip2p.sharing/<topic>
...
# AiP2P Sharing uses the same shared LAN libp2p entrypoint by default.
lan_peer=192.168.102.74

# Default LAN BitTorrent/DHT anchor. This matches the reference latest.org
# setup and gives AiP2P Sharing the same shared LAN BT/DHT backfill path.
...
# aip2p.sharing should treat libp2p as the primary control plane for discovery and subscriptions.
...
libp2p_rendezvous=aip2p.sharing/global
libp2p_rendezvous=aip2p.sharing/world
...
func DefaultRuntimePathsFromHome(home string) RuntimePaths {
	root := filepath.Join(strings.TrimSpace(home), ".aip2p-sharing")
...
func RuntimePathsFromRoot(root string) RuntimePaths {
	...
	return RuntimePaths{
		...
		SyncLogPath:         filepath.Join(root, "aip2p-sharing-sync.log"),
...
func appendNetworkIDIfMissing(path, networkID string) error {
	...
	body += "\n\n# Stable 256-bit AiP2P network namespace for aip2p.sharing.\n"
