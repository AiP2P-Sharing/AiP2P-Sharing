...
func (a *App) nodeStatus(index Index) NodeStatus {
	...
	discoveryTone := "warn"
	discoveryDetail := "Add aip2p_sharing_net.inf to declare bootstrap peers and routers."
	if netErr != nil {
		discoveryValue = "config error"
		discoveryTone = "bad"
		discoveryDetail = "aip2p_sharing_net.inf could not be read."
	...
	if netErr != nil {
		...
		networkIDTone = "bad"
		networkIDDetail = "network_id could not be parsed from aip2p_sharing_net.inf."
	...
	case netErr != nil:
		...
		summaryTone = "bad"
		summaryDetail = "aip2p_sharing_net.inf exists but could not be parsed."
	case len(netCfg.LibP2PBootstrap) > 0 && len(netCfg.DHTRouters) > 0:
		summary = "bootstrap ready"
		summaryTone = "good"
		summaryDetail = "libp2p and BitTorrent discovery profiles are loaded. AiP2P Sharing is still in UI/index mode until the sync daemon is running."
	case len(netCfg.LibP2PBootstrap) > 0 || len(netCfg.DHTRouters) > 0:
	...
	return NodeStatus{
		...
		Entries: []NodeStatusEntry{
			...
			{Label: "Bundle store", Value: fmt.Sprintf("%s · %d bundles", storeState, len(index.Bundles)), Detail: "AiP2P Sharing is reading from the local immutable bundle store.", Tone: storeTone},
...
func buildLiveNodeStatus(index Index, storeState, storeTone string, torrentCount int, netCfg NetworkBootstrapConfig, syncStatus SyncRuntimeStatus, listenAddr string) NodeStatus {
	...
	if !netCfg.Exists {
		...
		discoveryDetail = "Sync daemon is running, but aip2p_sharing_net.inf is not present on this node."
	}
