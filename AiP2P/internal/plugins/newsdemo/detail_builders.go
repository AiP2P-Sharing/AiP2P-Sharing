...
func BuildTypedCollectionPageData(app *App, index Index, spec CoordCollectionSpec, opts FeedOptions) TypedCollectionPageData {
	...
	return TypedCollectionPageData{
		...
		ModuleBoard:      ModuleCollectionBoard(spec.CoordType, allPosts),
		Path:             spec.Path,
...
func BuildPostPageData(app *App, index Index, navPath string, post Post, relatedCoordType string) PostPageData {
	return PostPageData{
		...
		ModuleReplyLanes:      ModuleReplyLanesForPost(post, index.RepliesByPost[strings.ToLower(post.InfoHash)]),
		ModuleReplyHighlights: ModuleReplyHighlightsForPost(post, index.RepliesByPost[strings.ToLower(post.InfoHash)]),
...
func BuildPostAPIResponse(app *App, index Index, scope string, post Post, relatedCoordType string) map[string]any {
	...
	return map[string]any{
		...
		"module_reply_lanes":      APIModuleReplyLanes(ModuleReplyLanesForPost(post, index.RepliesByPost[strings.ToLower(post.InfoHash)])),
		"module_reply_highlights": APIModuleReplyHighlights(ModuleReplyHighlightsForPost(post, index.RepliesByPost[strings.ToLower(post.InfoHash)])),
