...
func TestApplyWriterPolicyOnlyKeepsReadWriteOrigins(t *testing.T) {
	...
	postAllowed := Bundle{
		...
		Message: Message{
			...
			Extensions: map[string]any{"project": "aip2p.sharing", "topics": []any{"all", "world"}},
		},
	}
	postReadOnly := Bundle{
	...
	index := buildIndex([]Bundle{postAllowed, postReadOnly, replyReadOnly}, "aip2p.sharing")
	policy := WriterPolicy{
	...
	filtered := ApplyWriterPolicy(index, "aip2p.sharing", policy)
...
func TestApplyWriterPolicyWhitelistAcceptsOnlyExplicitWriters(t *testing.T) {
	...
	index := buildIndex([]Bundle{postAllowed, postOther}, "aip2p.sharing")
	policy := WriterPolicy{
...
func TestApplyWriterPolicyAllModeKeepsSignedReadOnlyWritersUnlessBlocked(t *testing.T) {
	...
	index := buildIndex([]Bundle{postReadOnly, postBlocked}, "aip2p.sharing")
	policy := WriterPolicy{
...
func TestApplyWriterPolicyAllModeRejectsUnsignedByDefault(t *testing.T) {
	...
	index := buildIndex([]Bundle{unsignedPost, noKeyPost}, "aip2p.sharing")
	policy := WriterPolicy{
		SyncMode:          WriterSyncModeAll,
		AllowUnsigned:     false,
		DefaultCapability: WriterCapabilityReadWrite,
...
func TestApplyWriterPolicyDelegationUsesParentCapability(t *testing.T) {
	...
	index := buildIndex([]Bundle{post}, "aip2p.sharing")
	policy := WriterPolicy{
	...
	filtered := ApplyWriterPolicyWithDelegations(index, "aip2p.sharing", policy, store)
	if len(filtered.Posts) != 1 {
		t.Fatalf("posts len = %d, want 1", len(filtered.Posts))
	}
